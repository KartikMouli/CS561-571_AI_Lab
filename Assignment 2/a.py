...
def A_Star(source_state, goal_state, heuristic):
    ...
    while open_list:
        ...
        if tuple(state) in closed_list:  # Convert list to tuple for checking in set
            pass
        else:
            pass
        ...
        # swap from up
        if empty_cell >= 3:
            state[empty_cell-3], state[empty_cell] = state[empty_cell], state[empty_cell - 3]
        ...
        if (empty_cell % 3 > 0):
            state[empty_cell-1], state[empty_cell] = state[empty_cell], state[empty_cell-1]
            if tuple(state) not in closed_list:
                if tuple(state) in states_exp:
                    if f[tuple(state)] > g + 1 + h(state, goal_state, heuristic):
                        new_h = h(state, goal_state, heuristic)
        ...
        if (empty_cell % 3 < 2):
            state[empty_cell +
                  1], state[empty_cell] = state[empty_cell], state[empty_cell + 1]
            if tuple(state) not in closed_list:
                if tuple(state) in states_exp:
                    if f[tuple(state)] > g + 1 + h(state, goal_state, heuristic):
                        new_h = h(state, goal_state, heuristic)
